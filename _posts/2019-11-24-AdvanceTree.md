...
以上就是删除代码的所有内容了,可以说是已经完成了一个平衡二叉树的绝大部分核心代码，以至于其他一些诸如**摧毁树**(一个后序遍历的删除操作)之类的比较容易一次性写对的操作就不再这里啰嗦了，AVL树到此告一段落。

**注意，这里的AVL树代码只是从思路的角度上来说没有问题，但是一些特定的功能还没有实现，也并没能测试是否真正能跑，只能作为考试应付用，不保证可以过题目。**
